...
#define EXR_CHECK(exr_result, result)\
	if (exr_result > EXR_ERR_SUCCESS)\
	{\
		printf("EXR ERR: %d %s %d\n", result, __FILE__, __LINE__);\
...
CHIZEN_RESULT write_exr(const char* file_path, const size_t render_width, const size_t render_height, const exr_pass* passes, const size_t passes_count)
{
	...
	EXR_CHECK(exr_start_write(&out, file_path, EXR_WRITE_FILE_DIRECTLY, &ctx_init), result);
	int part_id = 0;
	EXR_CHECK(exr_get_count(out, &part_id), result);

	int new_id = 0;
	EXR_CHECK(exr_add_part(out, "Render Layers", EXR_STORAGE_SCANLINE, &new_id), result);
	EXR_CHECK(exr_initialize_required_attr_simple(out, new_id, (int32_t)render_width, (int32_t)render_height, EXR_COMPRESSION_DWAA), result);
	...
	for (size_t p = 0; p < passes_count; ++p)
	{
		for (size_t c = 0; c < _countof(channel_names); ++c)
		{
			...
			EXR_CHECK(exr_add_channel(out, part_id, layer_channel_name, EXR_PIXEL_FLOAT, EXR_PERCEPTUALLY_LINEAR, 1, 1), result);
		}
	}

	EXR_CHECK(exr_write_header(out), result);

	int32_t chunk_count = 0;
	EXR_CHECK(exr_get_chunk_count(out, part_id, &chunk_count), result);

	int32_t scanlines_per_chunk = 0;
	EXR_CHECK(exr_get_scanlines_per_chunk(out, part_id, &scanlines_per_chunk), result);

	bool first = true;

	for (int32_t chunk_id = 0; chunk_id < render_height; chunk_id += scanlines_per_chunk)
	{
		EXR_CHECK(exr_write_scanline_chunk_info(out, part_id, chunk_id, &chunk_info), result);

		if (first)
		{
			EXR_CHECK(exr_encoding_initialize(out, part_id, &chunk_info, &encoder), result);
		}
		else
		{
			EXR_CHECK(exr_encoding_update(out, part_id, &chunk_info, &encoder), result);
		}
		...
		if (first)
			EXR_CHECK(exr_encoding_choose_default_routines(out, part_id, &encoder), result);

		EXR_CHECK(exr_encoding_run(out, part_id, &encoder), result);
		first = false;
	}

shutdown:
	EXR_CHECK(exr_encoding_destroy(out, &encoder), result);
	EXR_CHECK(exr_finish(&out), result);
