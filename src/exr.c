...
CHIZEN_RESULT write_exr(const char* file_path, const size_t render_width, const size_t render_height, const exr_pass* passes, const size_t passes_count)
{
	...
	EXR_CHECK("start write", exr_start_write(&out, file_path, EXR_WRITE_FILE_DIRECTLY, &ctx_init), chi_result);
	int part_id = 0;
	EXR_CHECK("get count", exr_get_count(out, &part_id), chi_result);

	int new_id = 0;
	EXR_CHECK("add part", exr_add_part(out, "Render Layers", EXR_STORAGE_SCANLINE, &new_id), chi_result);
	EXR_CHECK("init attr", exr_initialize_required_attr_simple(out, new_id, (int32_t)render_width, (int32_t)render_height, EXR_COMPRESSION_DWAA), chi_result);
	...
	for (size_t p = 0; p < passes_count; ++p)
	{
		for (size_t c = 0; c < passes[p].layer.num_channels; ++c)
		{
			...
			EXR_CHECK("add channel", exr_add_channel(out, part_id, layer_channel_name, EXR_PIXEL_FLOAT, EXR_PERCEPTUALLY_LINEAR, 1, 1), chi_result);
		}
	}

	EXR_CHECK("write header", exr_write_header(out), chi_result);

	int32_t chunk_count = 0;
	EXR_CHECK("get chunk count", exr_get_chunk_count(out, part_id, &chunk_count), chi_result);

	int32_t scanlines_per_chunk = 0;
	EXR_CHECK("get scanlines per chunk", exr_get_scanlines_per_chunk(out, part_id, &scanlines_per_chunk), chi_result);

	bool first = true;

	for (int32_t chunk_id = 0; chunk_id < render_height; chunk_id += scanlines_per_chunk)
	{
		EXR_CHECK("write scanline chunk info", exr_write_scanline_chunk_info(out, part_id, chunk_id, &chunk_info), chi_result);

		if (first)
		{
			EXR_CHECK("init encoding", exr_encoding_initialize(out, part_id, &chunk_info, &encoder), chi_result);
		}
		else
		{
			EXR_CHECK("update encoding", exr_encoding_update(out, part_id, &chunk_info, &encoder), chi_result);
		}
		...
		if (first)
			EXR_CHECK("choose default encoding routines", exr_encoding_choose_default_routines(out, part_id, &encoder), chi_result);

		EXR_CHECK("run encoding", exr_encoding_run(out, part_id, &encoder), chi_result);
		first = false;
	}

cpu_error:
	EXR_CHECK("destroy encoder", exr_encoding_destroy(out, &encoder), chi_result);
	EXR_CHECK("finish", exr_finish(&out), chi_result);
