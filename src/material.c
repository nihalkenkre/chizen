...
material material_create(const cgltf_data* gltf_data, cgltf_material* curr_mat, texture* textures)
{
	...
	if (curr_mat->has_pbr_metallic_roughness)
	{
		...
		{
			m.base_tex_idx = (int16_t)cgltf_texture_index(gltf_data, pbr_mr.base_color_texture.texture);
		}

		m.metalness_factor = pbr_mr.metallic_factor;
		if (pbr_mr.metallic_roughness_texture.texture != NULL)
		{
			m.mr_tex_idx = (int16_t)cgltf_texture_index(gltf_data, pbr_mr.metallic_roughness_texture.texture);
		}
	...
	{
		m.emissive_tex_idx = (int16_t)cgltf_texture_index(gltf_data, curr_mat->emissive_texture.texture);
	}
	...
	{
		m.transmission_factor = curr_mat->transmission.transmission_factor;
		
		if (curr_mat->transmission.transmission_texture.texture != NULL)
		{
