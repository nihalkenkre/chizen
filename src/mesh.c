...
mesh mesh_create(const cgltf_data* data, cgltf_mesh* curr_mesh, const OptixProgramGroup ch_rg_pg, const OptixProgramGroup ch_b_pg, const OptixProgramGroup ch_sr_pg, const OptixDeviceContext ctx, const cudaStream_t stream, ch_infos* ch_infos)
{
	...
	{
		m.prims[p] = primitive_create(data, curr_mesh->primitives + p, ch_rg_pg, ch_b_pg, ch_sr_pg, ctx, stream, ch_infos);
