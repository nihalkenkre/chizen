use std::{ffi::CStr, option, os::raw::c_void, u64};
...
//     pub fn new_gui_pipe(device: &Device, surf_extent: VkExtent2D) -> Self {
//         let stages = vec![
//             VkPipelineShaderStageCreateInfo::new(
//                 None,
//                 0,
//                 VK_SHADER_STAGE_VERTEX_BIT,
//                 match device.create_shader_module(
...
//                             Ok(x) => &x,
//                             Err(err) => {
//                                 panic!("ERR read gui vertex shader spv file {}", err);
//                             }
//                         },
//                     ),
//                     None,
...
//                     Ok(x) => x,
//                     Err(err) => {
//                         panic!("ERR create shader module {}", err);
//                     }
//                 },
//                 c"main",
//                 None,
...
//                             Ok(x) => &x,
//                             Err(err) => {
//                                 panic!("ERR read gui fragment shader spv file {}", err);
//                             }
//                         },
//                     ),
//                     None,
...
//         let pvis_ci = VkPipelineVertexInputStateCreateInfo::new(
//             None,
//             0,
//             &vec![
...
//             x: 0.0,
//             y: 0.0,
//             width: surf_extent.width as f32,
//             height: surf_extent.height as f32,
//             minDepth: 0.0,
//             maxDepth: 1.0,
//         };
...
//         let pvs_ci = VkPipelineViewportStateCreateInfo::new(None, 0, &pipe_viewports, &pipe_scissors)
//     }
// }
...
pub struct Vk {
    ...
    pub gfx_q: Queue,
    pub xfer_q: Queue,
    pub gui_pipe: GraphicsPipeline,
    pub scene_pipe: GraphicsPipeline,
}
...
impl Default for Vk {
    fn default() -> Self {
        Self {
            ...
            gui_pipe: GraphicsPipeline::default(),
            scene_pipe: GraphicsPipeline::default(),
        }
    }
}
...
impl Vk {
    pub fn init(window_handle: winit::raw_window_handle::RawWindowHandle) -> Self {
        ...
        let inst_ext_names = vec![
            VK_KHR_SURFACE_EXTENSION_NAME.to_owned(),
            VK_KHR_WIN32_SURFACE_EXTENSION_NAME.to_owned(),
        ];
        ...
        let instance = match Instance::create(
            VkInstanceCreateInfoBuilder::default()
                .application_info(
                    &VkApplicationInfoBuilder::default()
                        .application_name(c"Chizen")
                        .application_version(VK_MAKE_API_VERSION!(0, 1, 0, 0))
                        .engine_version(VK_MAKE_API_VERSION!(0, 1, 0, 0))
                        .api_version(VK_MAKE_API_VERSION!(0, 1, 3, 280))
                        .engine_name(c"Chizen")
                        .build(),
                )
                .extension_names(inst_ext_names)
                .build(),
            None,
        ) {
            Ok(x) => x,
            Err(err) => {
                panic!("ERR create instance {}", err);
            }
        };
        ...
        let surface = match instance.create_win32_surface_khr(
            VkWin32SurfaceCreateInfoKHRBuilder::default()
                .h_instance(match window_handle {
                    winit::raw_window_handle::RawWindowHandle::Win32(window) => {
                        window.hinstance.unwrap().get() as *mut HINSTANCE__
                    }
                    _ => std::ptr::null_mut(),
                })
                .h_wnd(match window_handle {
                    winit::raw_window_handle::RawWindowHandle::Win32(window) => {
                        window.hwnd.get() as *mut HWND__
                    }
                    _ => std::ptr::null_mut(),
        ...
        // let q_ci = VkDeviceQueueCreateInfo::new(
        //     None,
        //     0,
        //     q_fly_idx,
        //     q_fly_q_cnt,
        ...
        // ));
        // let q_cis = vec![q_ci];
        let dyn_rend_feats = VkPhysicalDeviceDynamicRenderingFeaturesBuilder::default()
            .p_next(std::ptr::addr_of!(feats2) as *mut c_void)
            .dynamic_rendering(1)
            .build();
        ...
        //     0,
        //     &vec![VkDeviceQueueCreateInfoBuilder::default()
        //         .q_family_index(q_fly_idx)
        //         .priorities(&vec![1.0; q_fly_q_cnt as usize])
        //         .build()],
        //     &dev_exts,
        //     None,
        // );
        ...
        let device = match Device::create(
            &phy_dev,
            VkDeviceCreateInfoBuilder::default()
                .p_next(std::ptr::addr_of!(dyn_rend_feats) as *const c_void)
                .queue_create_infos(&vec![VkDeviceQueueCreateInfoBuilder::default()
                    .q_family_index(q_fly_idx)
                    .priorities(&vec![1.0; q_fly_q_cnt as usize])
                    .build()])
                .enabled_extensions(vec![VK_KHR_SWAPCHAIN_EXTENSION_NAME.to_owned()])
                .build(),
            None,
        ) {
            Ok(device) => device,
        ...
        //     &q_fly_idxs,
        //     surf_caps.currentTransform as u32,
        //     VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR as u32,
        //     present_mode,
        //     1,
        //     None,
        // );

        let sc_img_cnt = surf_caps.minImageCount + 1;

        let swapchain = match device.create_swapchain_khr(
            VkSwapchainCreateInfoKHRBuilder::default()
                .surface(surface)
                .min_image_count(sc_img_cnt)
                .image_format(surf_format.format)
                .image_color_space(surf_format.colorSpace)
                .image_extent(surf_caps.currentExtent)
                .image_array_layers(1)
                .image_usage(surf_caps.supportedUsageFlags)
                .image_sharing_mode(VK_SHARING_MODE_EXCLUSIVE)
                .q_family_indices(&vec![q_fly_idx])
                .pre_tranform(surf_caps.currentTransform as u32)
                .composite_alpha(VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR as u32)
        ...
        let sc_images = match device.get_swapchain_images_khr(swapchain) {
            Ok(imgs) => imgs,
            Err(result) => {
        ...
        for sc_img in &sc_images {
            sc_image_views.push(
                match device.create_image_view(
                    VkImageViewCreateInfoBuilder::default()
                        .view_type(VK_IMAGE_VIEW_TYPE_2D)
                        .image(*sc_img)
                        .format(surf_format.format)
                        .subresource_range(
                            VkImageSubresourceRangeBuilder::default()
                                .aspect_mask(VK_IMAGE_ASPECT_COLOR_BIT as u32)
                                .level_count(1)
                                .layer_count(1)
                                .build(),
                        )
        ...
        let gui_pipe = GraphicsPipeline::default();
        let scene_pipe = GraphicsPipeline::default();
...
pub fn create_imgs_and_memory(
    ...
    for img_info in img_infos {
        ...
        let img = match device.create_image(
            VkImageCreateInfoBuilder::default()
                .image_type(img_info.img_type)
                ...
                .sharing_mode(img_info.sharing_mode)
                .q_family_indices(q_fly_idxs)
        ...
        if !mem_ai_found {
            ...
            mem_ais.push(
                VkMemoryAllocateInfoBuilder::default()
                    .allocation_size(mem_reqs.size)
                    .memory_type_index(mem_id)
                    .build(),
            );
