...
layout(set = 2, binding = 1) readonly buffer Positions
{
    vec3 positions[];
}
positions;
...
layout(set = 2, binding = 3) readonly buffer MeshletsVertices
{
    uint meshlets_vertices[];
}
meshlets_vertices;

layout(set = 2, binding = 4) readonly buffer MeshletsTriangles
{
...
void main()
{
    ...
    if (gl_LocalInvocationIndex < meshlet.vertex_count)
    {
        ...
        gl_MeshVerticesEXT[gl_LocalInvocationIndex].gl_Position = vp.vp * m.m * vec4(positions.positions[vertex_index], 1);
